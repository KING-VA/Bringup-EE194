...
module TLEToNoC(
  input        clock,
               reset,
               io_protocol_valid,
  input  [4:0] io_protocol_bits_sink,
  input        io_flit_ready,
  output       io_protocol_ready,
               io_flit_valid,
               io_flit_bits_head,
  output [4:0] io_flit_bits_payload,
               io_flit_bits_egress_id
);
  ...
  always @(posedge clock) begin
    if (reset)
      is_body <= 1'h0;	// @[Edges.scala:230:25, Tilelink.scala:59:24]
    else
  ...
  Queue_203 q (	// @[Tilelink.scala:45:17]
    .clock            (clock),
    .reset            (reset),
  ...
  assign io_flit_bits_payload = is_body ? 5'h0 : _q_io_deq_bits_sink;	// @[Mux.scala:27:73, Tilelink.scala:45:17, :59:24, :68:33]
  assign io_flit_bits_egress_id = {_q_io_deq_bits_sink[4:3] == 2'h2, (&(_q_io_deq_bits_sink[4:3])) ? 4'hD : 4'h0} | (_q_io_deq_bits_sink[4:3] == 2'h1 ? 5'h13 : 5'h0) | (_q_io_deq_bits_sink[4:3] == 2'h0 ? 5'h16 : 5'h0);	// @[Mux.scala:27:73, Parameters.scala:54:{10,32}, Tilelink.scala:45:17]
endmodule
