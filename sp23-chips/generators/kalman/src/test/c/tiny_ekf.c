...
static int choldc1(uint16_t * a, uint16_t * p, int n) {
    int i,j,k;
    uint16_t sum;
...
static int choldcsl(uint16_t * A, uint16_t * a, uint16_t * p, int n) 
{
    int i,j,k; uint16_t sum;
    for (i = 0; i < n; i++) 
        for (j = 0; j < n; j++) 
            a[i*n+j] = A[i*n+j];
    if (choldc1(a, p, n)) return 1;
...
static int cholsl(uint16_t * A, uint16_t * a, uint16_t * p, int n) 
{
    int i,j,k;
    if (choldcsl(A,a,p,n)) return 1;
    for (i = 0; i < n; i++) {
...
static void zeros(uint16_t * a, int m, int n)
{
    int j;
    for (j=0; j<m*n; ++j)
        a[j] = 0;
...
#ifdef DEBUG
static void dump(uint16_t * a, int m, int n, const char * fmt)
{
    int i,j;
...
/* C <- A * B */
static void mulmat(uint16_t * a, uint16_t * b, uint16_t * c, int arows, int acols, int bcols)
{
    int i, j,l;
...
static void mulvec(uint16_t * a, uint16_t * x, uint16_t * y, int m, int n)
{
    int i, j;
...
static void transpose(uint16_t * a, uint16_t * at, int m, int n)
{
    int i,j;
...
/* A <- A + B */
static void accum(uint16_t * a, uint16_t * b, int m, int n)
{        
    int i,j;
...
/* C <- A + B */
static void add(uint16_t * a, uint16_t * b, uint16_t * c, int n)
{
    int j;
...
/* C <- A - B */
static void sub(uint16_t * a, uint16_t * b, uint16_t * c, int n)
{
    int j;
...
static void negate(uint16_t * a, int m, int n)
{        
    int i, j;
...
static void mat_addeye(uint16_t * a, int n)
{
    int i;
    for (i=0; i<n; ++i)
        a[i*n+i] += 1;
...
typedef struct {
    ...
    uint16_t * x;    /* state vector */

    uint16_t * P;  /* prediction error covariance */
    uint16_t * Q;  /* process noise covariance */
    uint16_t * R;  /* measurement error covariance */

    uint16_t * G;  /* Kalman gain; a.k.a. K */

    uint16_t * F;  /* Jacobian of process model */
    uint16_t * H;  /* Jacobian of measurement model */

    uint16_t * Ht; /* transpose of measurement Jacobian */
    uint16_t * Ft; /* transpose of process Jacobian */
    uint16_t * Pp; /* P, post-prediction, pre-update */

    uint16_t * fx;  /* output of user defined f() state-transition function */
    uint16_t * hx;  /* output of user defined h() measurement function */
...
int ekf_step(void * v, uint16_t * z)
{        
    /* unpack incoming structure */
