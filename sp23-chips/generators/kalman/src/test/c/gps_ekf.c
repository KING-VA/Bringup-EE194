...
#else 
// positioning interval
static const uint16_t T = 1;

static void blkfill(ekf_t * ekf, const uint16_t * a, int off)
{
    off *= 2;
...
{
    // Set Q, see [1]
    const uint16_t Sf    = 36;
    const uint16_t Sg    = 0.01;
    const uint16_t sigma = 5;         // state transition variance
    const uint16_t Qb[4] = {Sf*T+Sg*T*T*T/3, Sg*T*T/2, Sg*T*T/2, Sg*T};
    const uint16_t Qxyz[4] = {sigma*sigma*T*T*T/3, sigma*sigma*T*T/2, sigma*sigma*T*T/2, sigma*sigma*T};
...
static void model(ekf_t * ekf, uint16_t SV[4][3])
{ 
...
static void readdata(FILE * fp, uint16_t SV_Pos[4][3], uint16_t SV_Rho[4])
{
    char line[1000];
    ...
    for (i=0; i<4; ++i)
        for (j=0; j<3; ++j) {
            SV_Pos[i][j] = fp16_alt_from_fp32_value(strtof(p, NULL));
            printf("NUM: %f", fp16_alt_to_fp32_value((u_int16_t) SV_Pos[i][j]));
            p = strtok(NULL, ",");
        }

    for (j=0; j<4; ++j) {
        SV_Rho[j] = fp16_alt_from_fp32_value(strtof(p, NULL));
        p = strtok(NULL, ",");
    }
}
...
{    
    ...
    uint16_t SV_Pos[4][3];
    uint16_t SV_Rho[4];
    uint16_t Pos_KF[25][3];

    // Open output CSV file and write header
    const char * OUTFILE = "ekf2.csv";
    FILE * ofp = fopen(OUTFILE, "w");
    fprintf(ofp, "X,Y,Z\n");
    ...
    for (j=0; j<25; ++j) {
        fprintf(ofp, "%f,%f,%f\n", 
                fp16_alt_to_fp32_value((u_int16_t) (Pos_KF[j][0]-mean_Pos_KF[0])), Pos_KF[j][1]-mean_Pos_KF[1], Pos_KF[j][2]-mean_Pos_KF[2]);
        printf("%f %f %f\n", Pos_KF[j][0], Pos_KF[j][1], Pos_KF[j][2]);
    }
