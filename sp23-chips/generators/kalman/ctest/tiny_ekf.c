...
static void mulmat(float * a, float * b, float * c, int arows, int acols, int bcols)
{
    ...
    if (acols == 8) {
        for(i=0; i<arows; ++i) {
            for(j=0; j<bcols; ++j) {
                float tmp1[8] = {a[i*acols], a[i*acols+1], a[i*acols+2], a[i*acols+3], a[i*acols+4], a[i*acols+5], a[i*acols+6], a[i*acols+7]};
                float tmp2[8] = {b[j], b[bcols+j], b[2*bcols+j], b[3*bcols+j], b[4*bcols+j], b[5*bcols+j], b[6*bcols+j], b[7*bcols+j]};
                c[i*bcols+j] = dot_vector_8(tmp1, tmp2);
            }
        }
    } else {
        for(i=0; i<arows; ++i) {
            for(j=0; j<bcols; ++j) {
                float tmp1[8] = {a[i*acols], a[i*acols+1], a[i*acols+2], a[i*acols+3]};
                float tmp2[8] = {b[j], b[bcols+j], b[2*bcols+j], b[3*bcols+j]};
                c[i*bcols+j] = dot_vector_4(tmp1, tmp2);
            }
        }
    }
}
...
{        
    ...
    float tmp1[8] = {-1, -1, -1, -1, -1, -1, -1, -1};

    for(i=0; i<m; ++i)
        mul_vector_8(&a[i*n], tmp1, &a[i*n]);
