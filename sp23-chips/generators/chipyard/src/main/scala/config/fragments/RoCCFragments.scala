...
class WithKalmanFilterMulti(harts: Int*) extends Config(
  new Config((site, here, up) => {
    case MultiRoCCKey => {
      up(MultiRoCCKey, site) ++ harts.distinct.map{ i =>
        (i -> Seq((p: Parameters) => {
          val kalman = LazyModule(new KalmanAccelerator(OpcodeSet.custom1)(p))
          kalman
        }))
      }
    }
...
class WithMultiLQR(harts: Int*) extends Config(
  new Config((site, here, up) => {
    case MultiRoCCKey => {
      up(MultiRoCCKey, site) ++ harts.distinct.map{ i =>
        (i -> Seq((p: Parameters) => {
...
class WithMultiSparseAccel(harts: Int*) extends Config(
  new Config((site, here, up) => {
    case MultiRoCCKey => {
      up(MultiRoCCKey, site) ++ harts.distinct.map{ i =>
        (i -> Seq((p: Parameters) => {
          val spacc = LazyModule(new spaccel.SparseAccel()(p))
          spacc
        }))
      }
    }
...
class WithMultiSparseAccelReserve(harts: Int*) extends Config(
  new Config((site, here, up) => {
    case MultiRoCCKey => {
      up(MultiRoCCKey, site) ++ harts.distinct.map{ i =>
        (i -> Seq((p: Parameters) => {
          val spacc = LazyModule(new spaccel_reserve.SparseAccel()(p))
          spacc
        }))
      }
    }
