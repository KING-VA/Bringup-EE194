...
trait HasChipyardPRCI { this: BaseSubsystem with InstantiatesTiles =>
  ...
  val tileResetSetter    = prci_ctrl_domain {
    TileResetSetter(prciParams.baseAddress + 0x0000, tlbus, tile_prci_domains.map(_.tile_reset_domain.clockNode.portParams(0).name.get), (1 until tile_prci_domains.length))
    // TileResetSetter(prciParams.baseAddress + 0x0000, tlbus, tile_prci_domains.map(_.tile_reset_domain.clockNode.portParams(0).name.get), Nil)
  }
  val tileClockGater     = prci_ctrl_domain {
    TileClockGater(prciParams.baseAddress + 0x1000, tlbus, prciParams.enableTileClockGating)
  }
  (aggregator
    := frequencySpecifier
    := clockGroupCombiner
