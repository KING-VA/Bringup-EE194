...
class WithTieOffSPI extends OverrideHarnessBinder({
  (system: HasPeripherySPIModuleImp, th: HasHarnessSignalReferences, ports: Seq[Data]) => {
    ports.foreach({ p => p := DontCare; })
  }
})
...
class WithClockAndResetFromHarness extends OverrideHarnessBinder({
  (system: HasChipyardPRCI, th: HasHarnessSignalReferences, ports: Seq[Data]) => {
    ...
    ports.map ({
      ...
      case r: AsyncReset => r := th.dutReset  // invert this
    })
  }
})

class WithClockAndResetFromHarnessWithDebugAndLock extends OverrideHarnessBinder({
  (system: HasChipyardPRCI, th: HasHarnessSignalReferences, ports: Seq[Data]) => {
    implicit val p = GetSystemParameters(system)
    ports.map ({
      case c: Clock => {
        c.name match {
          case "clock" => c := th.buildtopClock
          case "pll_in" => c := th.buildtopPllRef
          case "clock_debug" => th.buildtopClockDebug := c
          case _ => throw new Exception(s"Unknown clock ${c.name}")
        }
      }
      case r: AsyncReset => r := th.dutReset 
      case l: UInt => {
        th.buildtoppllLock := l
      }
    })
  }
})
