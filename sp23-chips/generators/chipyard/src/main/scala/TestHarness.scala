...
// Create a config fragment that changes the BuildTop to return sp23ChipTop
/*
case BuildTop => ((p: Parameters) => new ChipTop()(p))
*/
...
trait HasHarnessSignalReferences {
  ...
  def buildtopPllRef: Clock
  def buildtopClockDebug: Clock
  def buildtoppllLock: UInt
  def dutReset: Reset
  def success: Bool
}
...
class TestHarness(implicit val p: Parameters) extends Module with HasHarnessSignalReferences {
  ...
  val buildtopClockDebug = Wire(Clock())

  val buildtoppllLock = Wire(UInt(1.W))
  dontTouch(buildtoppllLock)
  dontTouch(buildtopClockDebug)

  val buildtopPllRef = Wire(Clock())
  ...
  val dutReset = (!buildtopReset.asBool).asAsyncReset
  val success = io.success

  // Eject of the following three lines
  ...
  buildtopPllRef := refClkBundle.clock
